...
const book2 = new Book("The Brief Wondrous Life of Oscar Wao", "Junot D\xEDaz", 339, "NOT READ", "#9d6838", "150", "450")
    ...
    for(let i = 0; i < library.length; i++){
        bookHtml += 
            `<div class="book" style="background-color:${library[i].color};width:${library[i].width}px;height:${library[i].height}px;">
                ...
                <div class="btns">
                    <button class="read-btn" onclick="toggleReadState(this.value)" value="${i}" title="Change READ status of book">&#128216</button>
                    <button class="trash-btn" onclick="deleteBook(this.value)" value="${i}" title="Remove book from library"><i class="fa fa-trash-o" style="font-size:26px"></i></button>
                </div>
            </div>`
    }
